...
public class RegularUtil {
	...
	/**
	 * [a-z0-9]
	 * 正则表达式判断字符串是否是6-16位密码
	 */
	public static boolean ispassworld(String strInput) {
		if (strInput != null && !strInput.trim().equalsIgnoreCase("")) {
	 ...
	 * @return
	 */
	public static boolean isPhone(String strInput) {
		if (strInput != null && !strInput.trim().equalsIgnoreCase("")) {
			String Regular = "^[1][0-9]{1}[0-9]{9}$";
...
//	 或 ^http://([\w-]+\.)+[\w-]+(/[\w-./?%&=]*)?$
	 ...
	 * @return
	 */
	public static boolean isInternetURL(String strInput) {
		if (strInput != null && !strInput.trim().equalsIgnoreCase("")) {
